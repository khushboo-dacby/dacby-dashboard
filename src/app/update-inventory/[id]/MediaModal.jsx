...
import { X, Trash2, Plus, Star, StarOff, AlertTriangle, Eye } from "lucide-react";
...
export default function MediaModal({ variant, isEditing, onClose, onApply }) {
  ...
  const colorStr = item.color || item.attributes?.color;
  const hasColor = Boolean(colorStr?.trim());
  ...
  function handleSave() {
    ...
    onApply(nextItem, { 
      applyToAllSameColor: applyToColor, 
      color: colorStr, 
      images,
      applyVideoToAll: true,
      yt_iframe: ytIframe.trim()
    });
  }
            ...
            <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-6 py-4">
              {hasColor ? (
                <label className="flex items-center gap-2 text-sm text-slate-600">
                  ...
                  />
                  Apply images to all variants with color <strong>{colorStr}</strong>
                </label>
              ) : <div />}
